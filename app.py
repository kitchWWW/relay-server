...
app = Flask(__name__)
# expose_headers lets browser JS read the Date response header (it is not
# CORS-safelisted by default), so clients can sync their clock to the server.
CORS(app, expose_headers=["Date"])
